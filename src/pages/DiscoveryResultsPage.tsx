...
export function DiscoveryResultsPage() {
  ...
  return (
    <div className="pb-24">
      ...
      <div className="mb-6 flex items-start justify-between">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <MapPin size={18} className="text-[#E07B3A]" />
            <h1 className="text-2xl font-bold text-[#F5EDD6]">
              Top {nicheLabel} Creators in {city}
            </h1>
          </div>
          <p className="text-sm text-[#C4A882]">
            {niche && <span className="font-medium text-[#F5EDD6]">{niche}</span>}
            {niche && ' · '}
            {results.length} creators found
            {sourceHashtags.length > 0 && (
              <span className="ml-2 text-[#7A6A54] text-xs">
      ...
      {locationFilterRelaxed && (
        <div className="mb-6 flex items-start gap-3 px-4 py-3.5 bg-[rgba(217,119,6,0.10)] border border-[#D97706]/40 rounded-xl">
