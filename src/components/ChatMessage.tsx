    ...
    <div className="flex items-end gap-2">
      ...
      <div className="flex flex-col gap-2 max-w-[80%]">
        ...
        {isError && onRetry && (
          <button
            onClick={onRetry}
            className="self-start flex items-center gap-1.5 px-3 py-1.5 text-xs text-[var(--color-error)] border border-[rgba(224,92,92,0.25)] rounded-lg hover:bg-[rgba(224,92,92,0.08)] transition-colors"
          >
            <RefreshCw size={11} />
            Retry
          </button>
...
export function TypingIndicator() {
  return (
    <div className="flex items-end gap-2">
      ...
      <div className="px-4 py-3 rounded-2xl rounded-bl-sm bg-surface border border-[rgba(var(--border-rgb),0.08)]">
  ...
  return (
    <div className="flex items-start gap-2">
      ...
      <div className="flex flex-col gap-2 min-w-[220px] max-w-[80%]">
        {label && (
          <div className="px-4 py-2.5 rounded-2xl rounded-tl-sm bg-surface border border-[rgba(var(--border-rgb),0.08)] text-sm text-secondary leading-relaxed">
            {label}
          </div>
        )}
        <div role="status" aria-live="polite" className="px-4 py-3 rounded-2xl rounded-tl-sm bg-surface border border-[rgba(var(--border-rgb),0.08)] flex flex-col gap-2.5">
          ...
          {elapsedSec ? (
            <span className="self-start text-[11px] font-mono text-[var(--color-text-muted)] tabular-nums">{formatElapsed(elapsedSec)} elapsed</span>
          ) : null}
          {onStop && (
            <button
              onClick={onStop}
              className="self-start flex items-center gap-1.5 mt-1 px-3 py-1.5 text-xs text-muted border border-[rgba(var(--border-rgb),0.10)] rounded-lg hover:text-secondary hover:border-[rgba(var(--border-rgb),0.2)] transition-colors"
            >
              <Square size={10} />
              Stop
            </button>
